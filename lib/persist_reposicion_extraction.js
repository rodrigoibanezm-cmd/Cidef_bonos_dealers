...
export async function persistReposicionExtraction(extraction) {
  ...
  const rows = await sql`
    INSERT INTO bonus_reposicion_extractions (
      tenant_id, file_id, source_filename, documento_valido,
      fecha, vin_nuevo, vin_original, nombre_dealer, rut_dealer,
    ...
    DO UPDATE SET
      ...
      updated_at = now()
    RETURNING id, tenant_id, file_id, source_filename, vin_nuevo, status, created_at, updated_at;
  `;
