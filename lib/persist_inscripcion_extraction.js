...
export async function persistInscripcionExtraction(extraction) {
  ...
  const identityEvidence = {
    nombre_adquirente: extraction.nombre_adquirente ?? null,
    rut_adquirente: extraction.rut_adquirente ?? null,
    vin: extraction.vin ?? null,
  };

  const rows = await sql`
    INSERT INTO bonus_inscripcion_extractions (
      tenant_id, file_id, source_filename, operation_vin, documento_valido, tipo_tramite, folio,
      numero_solicitud, fecha_solicitud, vin, ppu, marca, modelo, modelo_source, anio,
      nombre_adquirente, rut_adquirente, nombre_dealer, rut_dealer, identity_evidence,
    ...
    DO UPDATE SET
      ...
      updated_at = now()
    RETURNING id, tenant_id, file_id, source_filename, vin, modelo, modelo_source, numero_solicitud, status, created_at, updated_at;
  `;
