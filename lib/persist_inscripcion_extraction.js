...
export async function persistInscripcionExtraction(extraction) {
  ...
  const rows = await sql`
    INSERT INTO bonus_inscripcion_extractions (
      ...
      ${extraction.tipo_tramite ?? null},
      ${extraction.folio ?? null},
      ${extraction.numero_solicitud ?? null},
      ${extraction.fecha_solicitud ?? null},
      ${extraction.vin ?? null},
      ${extraction.ppu ?? null},
      ${extraction.marca ?? null},
      ${extraction.modelo ?? null},
      ${extraction.anio ?? null},
      ${extraction.nombre_adquirente ?? null},
      ${extraction.rut_adquirente ?? null},
      ${extraction.nombre_dealer ?? null},
      ${extraction.rut_dealer ?? null},
    ...
    DO UPDATE SET
      ...
      updated_at = now()
    RETURNING id, tenant_id, file_id, vin, numero_solicitud, status, created_at, updated_at;
  `;
