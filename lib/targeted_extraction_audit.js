...
export async function loadTargetedExtractionAudits({ tenantId, vin, issueCode, fileId, extractionId = null, sql = null }) {
  const dbSql = sql || db();
  return dbSql`
    select * from bonus_document_extraction_audits
    where tenant_id=${tenantId} and vin=${vin} and issue_code=${issueCode}
      and file_id=${fileId}
      and (${extractionId}::text is null or extraction_id=${extractionId}::text)
