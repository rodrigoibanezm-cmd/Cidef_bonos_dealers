...
function historicalClosureReview(value, bonoCierreLista) {
  const amount = money(value);
  return {
    amount,
    requiresReview: amount !== null && amount !== bonoCierreLista,
  };
}
...
export function calculateBonusBusinessRules(input) {
  ...
  const descuentosDealer = descuentoManual ?? (descuentoResidual !== null && descuentoResidual >= 0 ? descuentoResidual : null);
  ...
  const historical = historicalClosureReview(input.bono_cierre_historico, bonoCierreLista);
  const reviewReasons = [];
  ...
  if (descuentoResidual !== null && descuentoResidual < 0 && descuentoManual === null) reviewReasons.push("DESCUENTO_DEALER_RESIDUAL_NEGATIVO");
  ...
  const totalDeterministico = pdvOk === "OK"
    ...
    : null;
  const calculationStatus = override.requiresReview || historical.requiresReview || input.revision_pendiente === true || (descuentoResidual !== null && descuentoResidual < 0 && descuentoManual === null)
    ? "REQUIERE_REVISION"
