...
function historicalClosureReview(value, bonoCierreEfectivo) {
  ...
  return {
    amount,
    requiresReview: amount !== null && amount !== bonoCierreEfectivo,
  };
}
...
export function calculateBonusBusinessRules(input) {
  ...
  const bonoCierreResidual = calculateClosureResidual(input);
  ...
  const bonoCierreEfectivo = bonoCierreResidual !== null && !cierreResidualNegativo
    ? bonoCierreResidual
    : (descuentoManual === null ? bonoCierreLista : null);
  const cierreSource = bonoCierreResidual !== null && !cierreResidualNegativo
    ? "RESIDUAL_PDV"
    : (descuentoManual === null ? "LISTA" : null);

  const descuentoResidual = bonoCierreEfectivo === null
    ? null
    : calculateDealerDiscountResidual({ ...input, bono_cierre_venta: bonoCierreEfectivo });
  const descuentosDealer = descuentoManual ?? (
    descuentoResidual !== null && descuentoResidual >= 0 ? descuentoResidual : null
  );
  const descuentoSource = descuentoManual !== null ? "MANUAL" : descuentosDealer !== null ? "RESIDUAL" : null;
  const ruleVersion = input.rule_version ?? bonusRuleVersionForDate(input.fecha_venta);
  ...
  if (historical.requiresReview) reviewReasons.push("BONO_CIERRE_HISTORICO_DIFIERE_DE_CALCULO");
  ...
  if (descuentoResidual !== null && descuentoResidual < 0 && descuentoManual === null) {
    reviewReasons.push("DESCUENTO_DEALER_RESIDUAL_NEGATIVO");
  }
