...
export function calculateBonusBusinessRules(input) {
  ...
  // L (descuento dealer) is an independent business input. Never manufacture it
  // as the remainder of the PDV equation, because that consumes K by definition.
  const descuentoManual = money(input.descuentos_dealer);
  const descuentosDealer = descuentoManual;
  const descuentoSource = descuentoManual !== null ? "MANUAL" : null;

  // Once L is known, K is the deterministic residual that closes PDV:
  // K = H - I - J - L - F.
  ...
  // Diagnostic only: if K from list were used, what L would remain? This is not
  // approved or persisted as the dealer discount.
