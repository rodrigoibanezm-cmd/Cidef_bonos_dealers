...
function isReviewableState(state) {
  return !["PAGADA", "RECHAZADA"].includes(String(state || "").toUpperCase());
}

export function assertBonusRequestCalculationReady(request) {
  if (request?.requiere_revision_humana) {
    throw new Error("Request requires human resolution before final approval");
  }
  if (request?.pdv_ok !== "OK" || request?.total_devolver === null || request?.total_devolver === undefined) {
    throw new Error("Economic calculation must be complete before final approval");
  }
}
...
export async function getBonusRequestForReview(requestId, { sql = null } = {}) {
  ...
  const documents = await dbSql`
    select request_id,document_type,file_id,file_name,file_url,extraction,validation_status,review_status,
           reviewed_extraction,reviewed_by_user_id,reviewed_by_tenant_id,reviewed_at,updated_at
    from bonus_request_documents
    where request_id=${requestId}
  `;
  const reconstructionRows = await dbSql`
    select evidence->'fc_reconstruction' as fc_reconstruction
    from bonus_operation_closure_audits
    where tenant_id=${request.tenant_id} and upper(vin)=upper(${request.vin}) and phase='FINAL'
      and evidence ? 'fc_reconstruction'
    order by created_at desc limit 1
  `;

  const sequence = orderedSequence(documents);
  ...
  return {
    ...
    review_complete: orderedDocuments.length > 0 && !nextDocument,
    fc_reconstruction: reconstructionRows[0]?.fc_reconstruction || null,
  };
}
...
export async function approveBonusDocument({ requestId, documentType, reviewedExtraction }) {
  ...
  const updated = await sql`
    ...
    set review_status='APROBADO',
        reviewed_extraction=${JSON.stringify(finalExtraction)}::jsonb,
        reviewed_by_user_id=${TECHNICAL_REVIEW_ACTOR_USER},
        reviewed_by_tenant_id=${TECHNICAL_REVIEW_ACTOR_TENANT},
        reviewed_at=now(),
        updated_at=now()
    where request_id=${requestId}
  ...
  await sql`
    insert into bonus_request_events(
      ...
      ${requestId},${documentType},'DOCUMENTO_APROBADO',
      ${TECHNICAL_REVIEW_ACTOR_USER},${TECHNICAL_REVIEW_ACTOR_TENANT},
      ${JSON.stringify({ reviewed_extraction: finalExtraction, signature_status: "PENDING_FINAL_SIGNATURE" })}::jsonb
    )
  `;
  ...
  return {
    ...
    documents_complete: Boolean(after?.review_complete),
...
export async function approveBonusRequest({ requestId, actorUserId, actorTenantId }) {
  ...
  assertBonusRequestCalculationReady(review.request);
  ...
  await sql`
    update bonus_request_documents
    set reviewed_by_user_id=${actorUserId},
        reviewed_by_tenant_id=${actorTenantId},
        updated_at=now()
    where request_id=${requestId}
      and reviewed_by_user_id=${TECHNICAL_REVIEW_ACTOR_USER}
  `;

  await sql`
    insert into bonus_request_events(request_id,action,actor_user_id,actor_tenant_id,metadata)
    values(${requestId},'SOLICITUD_APROBADA',${actorUserId},${actorTenantId},'{}'::jsonb)
  `;
