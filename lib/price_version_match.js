...
export function scorePriceVersion(inventory, candidate) {
  ...
  const inventoryModel = norm(inventory?.modelo);
  const candidateIdentity = norm(`${candidate?.modelo || ""} ${candidate?.version || ""}`);
  const candidateIdentityTokens = new Set(candidateIdentity.split(/\s+/).filter(Boolean));
  let score = 0;
  const reasons = [];

  for (const token of inventoryModel.split(/\s+/).filter((value) => value.length >= 2)) {
    const discriminating = /^(?:E\d+|EV\d*|PRO|PLUS|LITE|PREMIUM|ULTIMATE)$/.test(token);
    if (discriminating ? !candidateIdentityTokens.has(token) : !has(candidateIdentity, token)) continue;
    score += discriminating ? 20 : 4;
    reasons.push(`inventario_modelo:${token}`);
  }
