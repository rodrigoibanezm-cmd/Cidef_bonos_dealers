...
export function normalizeOperationIdentity({ fv, ins }) {
  ...
  if (insRut) {
    const rutMatch = candidates.find((c) => normRut(c.rut) === insRut);
    if (rutMatch) {
      return {
        status: "RESOLVED",
  ...
  if (insName) {
    const nameMatch = candidates.find((c) => sameName(c.nombre, insName));
    if (nameMatch) {
      ...
      return {
        status: "UNRESOLVED",
        reason: "NAME_MATCH_RUT_CONFLICT",
        role: nameMatch.role,
        evidence,
      };
    }
  ...
  const preferred = candidates.find((c) => c.role === "COMPRA_PARA") || candidates.find((c) => c.role === "FACTURADO_A") || candidates[0];
  return {
