...
export async function listPendingBonusRequests({ urgentDays, limit = 200 }) {
  ...
  return sql`
    select
      ...
      greatest(current_date - coalesce(submitted_at, created_at)::date, 0)::int as dias,
      case
        when current_date - coalesce(submitted_at, created_at)::date >= ${urgentDays} then 'URGENTE'
        else upper(coalesce(estado, 'PENDIENTE'))
      end as estado_cola,
      estado
    from bonus_requests
    where coalesce(estado, '') <> 'PAGADA'
...
export async function listApprovedBonusRequestsByDealer({ tenantId, limit = 200 }) {
  ...
  return sql`
    select
      ...
      greatest(coalesce(approved_at, updated_at)::date - coalesce(submitted_at, created_at)::date, 0)::int as dias,
      estado
    from bonus_requests
