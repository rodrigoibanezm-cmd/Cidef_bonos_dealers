...
export async function readFcChassis(file) {
...
export async function extractFcChassis({ file, comparisonVin = null }) {
  ...
  const first = await readFcChassis(file);
  const needsRetry = !first.vin || !first.readable || first.parse_error || (comparison && first.vin !== comparison);
  ...
  const second = await readFcChassis(file);
  return {
    ...second,
    retried: true,
    first_vin: first.vin,
    retry_vin: second.vin,
