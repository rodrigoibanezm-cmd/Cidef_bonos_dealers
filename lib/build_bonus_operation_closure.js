...
function reconstructedFcEvidence(fc) {
  if (!fc?.fc_reconstruida) return null;
  return {
    source: fc.source,
    source_table: fc.source_table,
    documento_original: false,
    fc_reconstruida: true,
    vin: fc.vin,
    folio_factura_compra: fc.folio_factura_compra,
    fecha_factura_compra: fc.fecha_factura_compra,
    precio_compra_total: fc.precio_compra_total,
...
export function buildBonusOperationClosure({ vin, documents, identityResolution = null, exhaustedIssues = [], isFinal = false }) {
  ...
  const auditStatus = isFinal
    ...
    : "PENDIENTE_AUDITORIA";
  const informationalSignals = documents.fc?.fc_reconstruida
    ? [...(documents.fc.informational_signals || [])]
    : [];
  const fcReconstruction = reconstructedFcEvidence(documents.fc);
  ...
  return {
    ...
    informational_signals: informationalSignals,
    ...
    evidence: fcReconstruction ? { fc_reconstruction: fcReconstruction, informational_signals: informationalSignals } : {},
  };
}
