...
export async function persistFinanciamientoExtraction(extraction) {
  ...
  const rows = await sql`
    INSERT INTO bonus_financiamiento_extractions (
      tenant_id, file_id, source_filename, operation_vin, documento_valido, vin, marca, modelo, modelo_source, version, financiera,
      ...
      ${extraction.tenant_id}, ${extraction.file_id}, ${extraction.source_filename ?? null}, ${extraction.operation_vin ?? null}, ${extraction.documento_valido},
      ${extraction.vin ?? null}, ${extraction.marca ?? null}, ${extraction.modelo ?? null}, ${extraction.modelo_source ?? null},
    ...
    DO UPDATE SET
      ...
      updated_at = now()
    RETURNING id, tenant_id, file_id, source_filename, vin, marca, modelo, modelo_source, version, financiera, status, created_at, updated_at;
  `;
