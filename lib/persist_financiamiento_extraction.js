...
export async function persistFinanciamientoExtraction(extraction) {
  ...
  const rows = await sql`
    INSERT INTO bonus_financiamiento_extractions (
      tenant_id, file_id, source_filename, operation_vin, documento_valido, vin, marca, modelo, version, financiera,
    ...
    DO UPDATE SET
      ...
      updated_at = now()
    RETURNING id, tenant_id, file_id, source_filename, vin, marca, modelo, version, financiera, status, created_at, updated_at;
  `;
