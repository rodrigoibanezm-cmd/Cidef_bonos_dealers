...
export async function persistFinanciamientoExtraction(extraction) {
  ...
  const rows = await sql`
    INSERT INTO bonus_financiamiento_extractions (
      tenant_id, file_id, documento_valido, vin, financiera,
      ...
      ${extraction.vin ?? null}, ${extraction.financiera ?? null},
    ...
    DO UPDATE SET
      ...
      updated_at = now()
    RETURNING id, tenant_id, file_id, vin, financiera, status, created_at, updated_at;
  `;
