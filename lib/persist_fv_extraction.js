...
export async function persistFvExtraction(extraction) {
  ...
  const rows = await sql`
    INSERT INTO bonus_fv_extractions (
      ...
      ${extraction.file_id},
      ${extraction.vin ?? null},
      ${extraction.nombre_cliente ?? null},
      ${extraction.rut_cliente ?? null},
      ${extraction.nombre_dealer ?? null},
      ${extraction.rut_dealer ?? null},
      ${extraction.folio_factura_venta ?? null},
      ${extraction.fecha_factura_venta ?? null},
      ${extraction.precio_venta_total ?? null},
      ${extraction.financiamiento ?? null},
    ...
    DO UPDATE SET
      ...
      folio_factura_venta = EXCLUDED.folio_factura_venta,
      ...
      updated_at = now()
    RETURNING id, tenant_id, file_id, vin, status, created_at, updated_at;
  `;
