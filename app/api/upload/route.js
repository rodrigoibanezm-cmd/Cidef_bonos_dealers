...
export async function POST(request) {
  try {
    ...
    if (!file || typeof file === "string") return NextResponse.json({ ok: false, error: "file is required" }, { status: 400 });
    if (!vin) return NextResponse.json({ ok: false, error: "vin is required" }, { status: 400 });
    if (!ALLOWED_DOCUMENT_TYPES.has(documentType)) return NextResponse.json({ ok: false, error: "invalid document_type" }, { status: 400 });
    if (!ALLOWED_MIME_TYPES.has(file.type)) return NextResponse.json({ ok: false, error: "Formato no soportado. Usa PDF, JPG, PNG o WEBP." }, { status: 400 });
    if (documentType === "CARTA" && !expectedRut) return NextResponse.json({ ok: false, error: "expected_rut is required for CARTA" }, { status: 400 });
    ...
    const input = { tenantId, fileId: uploaded.id, expectedVin: vin, file: { base64: bytes.toString("base64"), mimeType: file.type } };
    ...
    if (valid && documentType !== "REPOS") {
      ...
      if (!requestId) {
        if (documentType !== "FC") return NextResponse.json({ ok: false, error: "No se encontró la solicitud abierta para este VIN" }, { status: 400 });
    ...
    return NextResponse.json({ ok: true, document_type: documentType, vin, request_id: requestId, persisted: valid && documentType !== "REPOS", file: uploaded, extraction });
  } catch (error) {
    console.error("Upload/extraction failed", error);
    return NextResponse.json({ ok: false, error: error?.message || "Upload/extraction failed" }, { status: 500 });
  }
}
