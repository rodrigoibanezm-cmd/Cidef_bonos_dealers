...
function AuditSummary({ request }) {
  const cierre=String(request.cierre_estado||"").toUpperCase(); const inconsistencias=asList(request.inconsistencias); const tone=cierre==="VERDE"?styles.auditGreen:cierre==="AMARILLO"?styles.auditYellow:styles.auditRed;
...
function PriceSummary({ request, auditors }) {
  return <section className={styles.priceSummary}><div className={styles.priceHeader}><div><small>Cálculo final</small><strong>Origen del precio y bonos</strong></div><span>{request.price_lookup_status||"SIN LOOKUP"}</span></div><div className={styles.priceGrid}><div><small>Lista de precios utilizada</small><strong>{request.lista_precio_utilizada||"-"}</strong></div><div><small>Versión precio</small><strong>{request.price_version_id||"-"}</strong></div><div><small>Precio lista venta</small><strong>{money(request.precio_lista_venta)}</strong></div><div><small>Bono CIDEF / cierre lista</small><strong>{money(request.bono_cierre_venta)}</strong></div><div><small>Bono financiamiento lista</small><strong>{money(request.bono_fin_venta)}</strong></div><div><small>Diferencia precio calculada</small><strong>{money(request.bono_dif)}</strong></div><div><small>Bono cierre a pagar</small><strong>{money(request.bono_cierre)}</strong></div><div><small>Bono financiamiento a pagar</small><strong>{money(request.bono_fin)}</strong></div><div className={styles.priceTotal}><small>Total devolución</small><strong>{money(request.total_devolver)}</strong></div></div><FinalApprovalClient requestId={request.id} auditors={auditors}/></section>;
}
