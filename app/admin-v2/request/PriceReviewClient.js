...
export default function PriceReviewClient({request,priceEvidence}){
  ...
  return <><div className={styles.documentHeader}><div><strong>PRECIOS · Paso final de validación</strong><span>{request.lista_precio_utilizada||"Lista de precios no identificada"}</span></div><span className={styles.pending}>VALIDAR</span></div><section className={styles.grid}><div className={styles.visualPanel}><div className={styles.panelTitle}>Lista de precios original</div>{request.lista_precio_utilizada?<XlsViewport requestId={request.id} filename={request.lista_precio_utilizada} modelLabel={modelLabel}/>:<div className={styles.noFile}>No existe una lista de precios vinculada al cálculo.</div>}</div><div className={styles.dataPanel}><div className={styles.panelTitle}>Campos a validar</div><div className={styles.fields}>{READ_ONLY_FIELDS.slice(0,2).map(([key,label,kind])=><div className={styles.fieldRow} key={key}><div className={styles.fieldText}><small>{label}</small><strong>{display(request[key],kind)}</strong></div></div>)}{EDITABLE_FIELDS.map(([key,label])=><div className={styles.fieldRow} key={key}><div className={styles.fieldText}><small>{label}</small><input type="number" min="0" step="1" value={values[key]} onChange={(e)=>setValues((current)=>({...current,[key]:e.target.value}))}/></div></div>)}{READ_ONLY_FIELDS.slice(2).map(([key,label,kind])=><div className={styles.fieldRow} key={key}><div className={styles.fieldText}><small>{label}</small><strong>{display(request[key],kind)}</strong></div></div>)}</div>{message?<p className={styles.error}>{message}</p>:null}<button type="button" className={styles.approve} disabled={busy} onClick={recalculate}>{busy?"Recalculando...":"Recalcular con montos corregidos"}</button><a className={styles.continueButton} href={summaryHref}>Validar precios y ver consolidado →</a></div></section></>;
}
