...
function moneyInput(value){if(value===null||value===undefined||value==="")return"";const digits=String(value).replace(/\D/g,"");return digits?`$ ${Number(digits).toLocaleString("es-CL")}`:"";}
function rawMoney(value){const digits=String(value??"").replace(/\D/g,"");return digits?Number(digits):"";}
...
function Applicability({request,field}){if(!BONUS_FIELDS.has(field))return null;const applies=bonusApplies(request,field);return <span className={applies?styles.approved:styles.pending}>{applies?"APLICA":"NO APLICA"}</span>;}
...
export default function PriceReviewClient({request,priceEvidence}){
  ...
  const priceFilename=request.lista_precio_utilizada||priceEvidence?.source_file||"";
  const displayedRequest={
    ...request,
    lista_precio_utilizada:priceFilename||request.lista_precio_utilizada,
    price_version_id:request.price_version_id??priceEvidence?.price_version_id,
  };
  ...
  async function recalculate(){if(busy)return;setBusy(true);setMessage("");try{const response=await fetch("/api/admin/requests/recalculate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({request_id:request.id,...Object.fromEntries(Object.entries(values).map(([key,value])=>[key,rawMoney(value)]))})});const result=await response.json();if(!response.ok||!result.ok)throw new Error(result.error||"No fue posible recalcular");setMessage("Cálculo actualizado");router.refresh();}catch(error){setMessage(error.message||"No fue posible recalcular");}finally{setBusy(false);}}
  return <><div className={styles.documentHeader}><div><strong>PRECIOS · Paso final de validación</strong><span>{priceFilename||"Lista de precios no identificada"}</span></div><div className={styles.documentNav}><button type="button" className={styles.editButton} onClick={()=>router.back()}>← Anterior</button><span className={styles.pending}>VALIDAR</span></div></div><section className={styles.grid}><div className={styles.visualPanel}><div className={styles.panelTitle}>Lista de precios original</div>{priceFilename?<XlsViewport requestId={request.id} filename={priceFilename} modelLabel={modelLabel}/>:<div className={styles.noFile}>No existe una lista de precios vinculada al cálculo.</div>}</div><div className={styles.dataPanel}><div className={styles.panelTitle}>Campos a validar</div><div className={styles.fields}>{READ_ONLY_FIELDS.slice(0,2).map(([key,label,kind])=><div className={styles.fieldRow} key={key}><div className={styles.fieldText}><small>{label}</small><strong>{display(displayedRequest[key],kind)}</strong></div></div>)}{EDITABLE_FIELDS.map(([key,label])=><div className={styles.fieldRow} key={key}><div className={styles.fieldText}><small>{label}</small><input type="text" inputMode="numeric" value={moneyInput(values[key])} onChange={(e)=>setValues((current)=>({...current,[key]:rawMoney(e.target.value)}))}/></div><Applicability request={request} field={key}/></div>)}{READ_ONLY_FIELDS.slice(2).map(([key,label,kind])=><div className={styles.fieldRow} key={key}><div className={styles.fieldText}><small>{label}</small><strong>{display(request[key],kind)}</strong></div><Applicability request={request} field={key}/></div>)}</div>{message?<p className={styles.error}>{message}</p>:null}<button type="button" className={styles.approve} disabled={busy} onClick={recalculate}>{busy?"Recalculando...":"Recalcular con montos corregidos"}</button><div className={styles.documentNav}><button type="button" className={styles.editButton} onClick={()=>router.back()}>← Anterior</button><a className={styles.continueButton} href={summaryHref}>Validar precios y ver consolidado →</a></div></div></section></>;
}
