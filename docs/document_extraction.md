...
Si el dealer no aporta FC y no existe una fila en `bonus_fc_extractions`, la consolidación puede reconstruir evidencia interna desde `vehiculos_raw`, complementada por `ventas_raw`. Sólo aplica cuando existe una única fila con VIN exacto, el RUT resuelve de forma determinística contra `dealers_master`, el dealer es consistente con la FV y existen factura, fecha, monto con IVA, nota de venta, marca y modelo completos.
